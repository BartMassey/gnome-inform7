...
on_navigation_requested(WebKitWebView *webview, WebKitWebFrame *frame, WebKitNetworkRequest *request, I7Panel *panel)
{
	...
	if(strcmp(scheme, "about") == 0) {
		...
		g_strfreev(elements);
		gchar **tryloc = g_new0(gchar *, 4);
		tryloc[0] = g_build_filename("Documentation", "Sections", tail, NULL);
		tryloc[1] = g_build_filename("Documentation", tail, NULL);
		tryloc[2] = g_build_filename(g_get_home_dir(), "Inform", "Documentation", tail, NULL);
		tryloc[3] = NULL;
		g_free(tail);
		
		for(foo = 0; foo < 3; foo++)
			if(foo < 2 && i7_app_check_datafile(theapp, tryloc[foo])) {
				gchar *filename = i7_app_get_datafile_path(theapp, tryloc[foo]);
				html_load_file(webview, filename);
				g_free(filename);
				break;
			} else if(foo == 2 && g_file_test(tryloc[foo], G_FILE_TEST_EXISTS)) {
				html_load_file(webview, tryloc[foo]);
				break;
			}
		g_strfreev(tryloc);
